...
{
	// process each mesh locates at the current node
...
std::vector<Texture> Model::loadMaterialTextures(aiMaterial* mat, aiTextureType type, std::string typeName)
{
	...
	for (unsigned int i = 0; i < mat->GetTextureCount(type); i++)
	{
		...
		// check if texture was loaded before and if so, continue iteration: skip loading a new texture
		bool skip = false;
		...
		if (!skip)
		{
			// if texture hasn't been laoded already, load it
			Texture texture;
